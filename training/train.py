...
def train(
    model, optimizer, criterion, weights_orthogonalizer, fmap_criterion, prev_ce_criterion,
    regularization, scheduler, train_loader, valid_loader, args, mixup_fn, loss_scaler
):
    ...
    for epoch_counter in range(args.start_epoch, total_epochs + 1):
        # if args.wo_beta != 0:
        #     weights_orthogonalizer.new_epoch()
        ...
        running_ce_loss, running_fmap_loss, running_wo_loss, running_prev_ce_loss, running_reg_loss, running_acc = (
        ...
        for n_iter, (images, labels) in enumerate(tqdm(train_loader)):
            ...
            with torch.cuda.amp.autocast(enabled=args.amp_enable):
                preds, f_map = model(images, True)
                regularizer_f_map = fmap_criterion.regularizer_model_forward(images)
                random_prev_preds = None if args.prev_ce_beta == 0 else prev_ce_criterion.random_model_forward(images)
            ce_loss = criterion(preds, labels)
            fmap_loss, losses_xz, losses_zx, losses = fmap_criterion(f_map, regularizer_f_map)
            
            wo_loss = torch.tensor([0.0], device=args.device) 
            if args.wo_beta != 0:
                wo_loss = args.wo_beta * weights_orthogonalizer(model)
            prev_ce_loss = torch.tensor([0.0], device=args.device) 
            if args.prev_ce_beta != 0:
                prev_ce_loss = args.prev_ce_beta * prev_ce_criterion(preds, random_prev_preds, labels)
            ...
            optimizer.zero_grad()
            loss = ce_loss + reg_loss + fmap_loss + wo_loss + prev_ce_loss
            ...
            running_fmap_loss += fmap_loss.detach() 
            running_wo_loss += wo_loss.detach() 
            running_prev_ce_loss += prev_ce_loss.detach() 
            ...
            if steps_counter % args.wandb_log_rate == 0:
                if args.use_wandb:
                    wandb.log({
                        ...
                        'batch_fmap_loss': fmap_loss.item(),
                        'batch_wo_loss': wo_loss.item(),
                        'batch_prev_ce_loss': prev_ce_loss.item(),
        ...
        if args.use_wandb:
            wandb.log({'train_loss': running_ce_loss.item() / len(train_loader.dataset),
                       'train_fmap': running_fmap_loss.item() / len(train_loader.dataset),
                       'train_wo': running_wo_loss.item() / len(train_loader.dataset),
                       'train_prev_ce': running_prev_ce_loss.item() / len(train_loader.dataset),
                       'train_reg': running_reg_loss.item() / len(train_loader.dataset),
        ...
        if epoch_counter in checkpoint_epochs:
            ...
            if args.wo_beta != 0:
                weights_orthogonalizer.insert_model(model)
            fmap_criterion.copy_if_none_regularizer_model(model)
            fmap_criterion.activate()
            prev_ce_criterion.copy_regularizer_model(model)
            if num_fge > 1:
                ...
                if getattr(args, 'star_fge', False):
                    ...
                    trainable_parameters_cnt = sum([np.prod(p.size()) for p in trainable_parameters])
                    print(f"trainable_parameters_cnt: {trainable_parameters_cnt}")
            else:
