...
def build_optimizer(config, model, simmim=False):
    ...
    if 'swin' in config.model.lower():
        ...
        # parameters_cnt = sum([np.prod(p.size()) for p in parameters])
        # print(f"parameters_cnt: {parameters_cnt}")
        # print(parameters)
        parameters = []
        ...
        trainable_parameters_cnt = sum([np.prod(p.size()) for p in parameters])
