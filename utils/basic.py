...
def init_wandb(args):
    if not getattr(args, 'exp_name', ''):
        exp_name = \
            f'Train_model={args.model}-dataset={args.dataset}-sched={args.scheduler}-' \
            f'fge_lr={args.fge_lr}-wd={args.wd}-fge_epochs={args.fge_epochs}'
        # fmap
        if max([abs(beta) for beta in args.fmap_betas]) > 0:
            for i in range(len(args.fmap_betas)):
                if args.fmap_betas[i] != 0:
                    exp_name += f"-fmap{i + 1}={args.fmap_betas[i]}"
        # prev_ce
        if args.prev_ce_beta != 0:
            exp_name += f"-prev_ce_beta={args.prev_ce_beta}-only_negative={args.only_negative}"
        # wo
        if args.wo_beta != 0:
            exp_name += f"-wo_beta={args.wo_beta}"
        exp_name += '.pt'
    else:
